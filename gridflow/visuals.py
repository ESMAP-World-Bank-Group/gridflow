...
def country_viz(terr):
    ...
    subregions_p = terr.subregions.to_crs(epsg=3857)
    lines_p = terr.grid.lines.to_crs(epsg=3857)
    subregions_p["centers"] = subregions_p.geometry.centroid

    subregions_p.plot(edgecolor="black", color="lightblue", ax=ax)
    lines_p.plot(color="pink", ax=ax)

    for idx, r in subregions_p.iterrows():
    ...
    ridx = subregions_p.index
    for i in ridx:
        for j in ridx:
            ci = subregions_p.centers.loc[i]
            cj = subregions_p.centers.loc[j]
